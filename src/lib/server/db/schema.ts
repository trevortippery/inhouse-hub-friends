...
export const participant = pgTable('participant', {
	...
	gameName: text('game_name').notNull().unique(), // Unique game name/IGN
...
export const match = pgTable('match', {
	id: uuid().defaultRandom().primaryKey(),
	matchNumber: serial('match_number').notNull(),
	...
	updatedAt: timestamp('updated_at')
		...
		.$onUpdate(() => /* @__PURE__ */ new Date())
		.notNull(),
	winningTeam: text('winning_team', { enum: ['blue', 'red', 'none'] }).default('none'),
	gameDuration: integer('game_duration'), // in seconds or minutes
...
export const matchParticipant = pgTable(
	'match_participant',
	{
		...
		participantId: uuid('participant_id')
			.notNull()
			.references(() => participant.id, { onDelete: 'restrict' }), // Changed from storing gameName directly
		team: text('team', { enum: ['blue', 'red'] }).notNull(),
		role: text('role', { enum: ['top', 'jungle', 'mid', 'bot', 'support'] }).notNull(),
	...
	(table) => [
		...
		index('match_participant_participantId_idx').on(table.participantId)
	]
);
...
export const matchParticipantRelations = relations(matchParticipant, ({ one }) => ({
	...
	participant: one(participant, {
		fields: [matchParticipant.participantId],
